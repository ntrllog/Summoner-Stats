...
from exports import championIdMap, queueTypes, itemMap, currentPatch
...
PROFILE_ICON = f'http://ddragon.leagueoflegends.com/cdn/{currentPatch}/img/profileicon/'
...
CHAMPION_SQUARE_ICON = f'http://ddragon.leagueoflegends.com/cdn/{currentPatch}/img/champion/'
...
ITEM_URL = f'http://ddragon.leagueoflegends.com/cdn/{currentPatch}/img/item/'
...
def showProfile(name):
    ...
    # get ranked info only if player has played any ranked games
    if parsedRankedDataList:
        # only grab ranked solo (not ranked flex)
        for parsedRankedData in parsedRankedDataList:
            if parsedRankedData['queueType'] == 'RANKED_SOLO_5x5':
    ...
    parsedMatchList = parsedMatchList['matches'][:5]
    ...
    for parsedMatch in parsedMatchList:
        ...
        for i in range(10):
            ...
            # get player stats
            if summonerData['id'] == playerIdentity['summonerId']:
